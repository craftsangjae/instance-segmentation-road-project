...
https://github.com/fchollet/keras/blob/master/keras/setup/training_utils.py
...
class ParallelModel(Model):
    ...
    def __init__(self, keras_model, gpu_count):
        ...
        """
        merged_outputs = make_parallel(keras_model, gpu_count)
        ...
        with tf.device('/gpu:%d' % i):
            with tf.name_scope('tower_%d' % i):
                ...
                zipped_inputs = zip(keras_model.input_names,
                                    keras_model.inputs)
                inputs = [Lambda(lambda s: input_slices[name][i],
